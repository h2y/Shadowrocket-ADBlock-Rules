...
这个规则和上面的 `黑名单 + 广告规则` 类似，但区别在于这是一个手工编写的规则，已经维护了一年多。而自动生成脚本是 2017-06 新开发的，或许还不够成熟，所以目前而言仍然推荐选用此规则。
